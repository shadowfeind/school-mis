...
import {
    getAllAnnouncementAction, getSingleAnnouncementAction,
...
const useStyles = makeStyles((theme) => ({
  ...
  const AnnouncementTest=()=> {
    ...
    const {
      ...
      const { singleAnnouncement, error: singleAnnouncementError } =
        useSelector((state) => state.getSingleAnnouncement);
    ...
    const {
        TableContainer, TblHead, TblPagination, tableDataAfterPagingAndSorting,
        ...
        setFilterFn({
            fn: (item) => {
                if (e.target.value === "") {
                    ...
                    return item.filter((x) => x.NewsHeading.toLowerCase().includes(e.target.value)
                    );
                }
            },
        });
