...
import { GET_ALL_GUARDIAN_RESET } from "./GuardianConstants";
import {
  getAllGuardianAction,
  getSingleGuardianAction,
...
const Guardian = () => {
  ...
  const { getAllGuardian, error } = useSelector(
    (state) => state.getAllGuardian
  );
  ...
  return (
    <CustomContainer>
      ...
      {getAllGuardian && (
        <ListGuardian list={getAllGuardian && getAllGuardian.dbModel} />
      )}
      <Popup
        openPopup={openPopup}
      ...
      >
        {" "}
        <GuardianForm
          guardianForm={singleGuardian && singleGuardian.dbModel}
          setOpenPopup={setOpenPopup}
        />
      </Popup>
